...
if not st.session_state.autenticado:
    ...
    <style>
    .stApp {{
        background-image: linear-gradient(rgba(0,0,0,0.55), rgba(0,0,0,0.55)), url("data:image/png;base64,{bg_base64}");
    ...
    with col_centro:
        ...
        with tab_cadastro:
            ...
            with col_cep_btn:
                st.markdown("<div style='margin-top: 28px; background-color:  linear-gradient(90deg, #1597e5, #67C5F2) !important;'></div>", unsafe_allow_html=True)
...
for idx, p in enumerate(st.session_state.pontos_ensaio):
  ...
  with col_r1:
    #st.text(p["id"])
    st.markdown(f"<div class='titulo-tabela'>{p['id']}</div>", unsafe_allow_html=True)
  with col_r2:
  ...
  with col_r3:
    ...
    st.markdown(f'<div class="titulo-tabela">{i_sec:.2f} A</span>', unsafe_allow_html=True)
  with col_r4:
    ...
    st.markdown(f'<div class="titulo-tabela">{t_teorico_str}</span>', unsafe_allow_html=True)
